...
class MazeImgEnv(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    def __init__(
        self, maze_spec=U_MAZE, reward_type="dense", reset_target=False, **kwargs
    ):
        offline_env.OfflineEnv.__init__(self, **kwargs)
    ...
    def step(self, action):
        ...
        img = self._get_obs()
        return img, reward, done, dict(goal_achieved=goal_achieved)

    def _get_ori_obs(self):
        obs = np.concatenate(
            [self.sim.data.qpos, self.sim.data.qvel, self._target]
        ).ravel()
        return obs 
    ...
    def viewer_setup(self):
        ...
        self.viewer.cam.trackbodyid = 0  # id of the body to track ()
        self.viewer.cam.lookat[0] = 4.5
        self.viewer.cam.lookat[1] = 4.5
        self.viewer.cam.lookat[2] = 0.5
